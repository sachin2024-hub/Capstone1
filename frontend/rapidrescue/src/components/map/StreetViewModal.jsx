...
export default function StreetViewModal({ target, onClose, portalTarget = null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const heading = road ? headingBetween(lookAt, origin) : 0;
      if (cancelled) return;
      ...
      if (key) {
        try {
          ...
          const data = await findNearbyStreetView(google, lookAt.lat, lookAt.lng);
          if (cancelled) return;
          if (!data) {
            setView({ ...lookAt, heading });
            setMode('embed');
            return;
          }
      ...
      <div className={styles.sheet}>
        <div className={styles.header}>
          <div className={styles.headerText}>
            <div className={styles.titleRow}>
              ...
              <h2>Nearby Street View</h2>
            </div>
            <p className={styles.subtitle}>
              {target.label || 'Victim / Help location'}
            </p>
          </div>
          <p className={styles.hint}>📍 Click and drag to look around</p>
        ...
        <div className={styles.body}>
          ...
          {mode === 'embed' && view && (
            <iframe
              ...
              src={embedUrl(view.lat, view.lng, view.heading)}
        ...
        <div className={styles.footer}>
          <p className={styles.note}>
            The imagery represents the nearest available road view and may not show the exact incident location.
          </p>
